...
pub struct DigitalInputGroup<'a, 'b, const INPUTS_SIZE: usize, M: RawMutex, T: 'static + PrimInt, const RECEIVER_SIZE: usize> {
    inp: [DigitalInput<'static>; INPUTS_SIZE],
    sender: Option<&'a watch::Sender<'b, M, T, RECEIVER_SIZE>>,
}

impl<'a, 'b, const INPUTS_SIZE: usize, M: RawMutex, T: 'static + PrimInt, const RECEIVER_SIZE: usize> DigitalInputGroup<'a, 'b, INPUTS_SIZE, M, T, RECEIVER_SIZE> {
    pub fn new(
        inp: [DigitalInput<'static>; INPUTS_SIZE],
